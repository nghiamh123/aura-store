...
export default function AdminDashboard() {
  ...
  // Blog posts state
  const [posts, setPosts] = useState<Array<{ id: number; title: string; slug: string; excerpt: string; coverImage?: string | null; createdAt: string }>>([]);
  ...
  const loadPosts = useCallback(async () => {
    try {
      ...
      const data = await apiFetch<{ posts: Array<{ id: number; title: string; slug: string; excerpt: string; coverImage?: string | null; createdAt: string }> }>(
        '/posts',
        { method: 'GET' }
      );
      setPosts(data.posts || []);
  ...
  async function createPost() {
    try {
      ...
      setPostForm({ title: '', slug: '', excerpt: '', content: '', coverImage: '', tags: '', status: 'DRAFT' });
      setCoverFile(null);
      setPostFiles([]);
      await loadPosts();
    } catch (e: unknown) {
            ...
            {activeTab === 'blog' && (
              <div>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => setShowCreatePost(true)} className="flex items-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
                    <Plus className="h-4 w-4 mr-2" />
                    Thêm bài viết
                  </button>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {posts.map((p) => (
                      <div key={p.id} className="bg-white border rounded-xl overflow-hidden">
                        ...
                        <div className="p-4">
                          <div className="text-sm text-gray-500 mb-1">{new Date(p.createdAt).toLocaleDateString('vi-VN')}</div>
                          <div className="font-semibold text-gray-900 line-clamp-2">{p.title}</div>
                          <div className="mt-3 text-right">
                            <a href={`/blog/${p.slug}`} target="_blank" className="text-purple-600 hover:text-purple-700 text-sm">Xem</a>
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="mt-6 bg-white border rounded-xl p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <input value={postForm.title} onChange={(e) => setPostForm({ ...postForm, title: e.target.value })} className="w-full px-3 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-700" />
                      </div>
                      <div>
                        <label className="block text-sm text-gray-700 mb-1">Slug *</label>
                        <input value={postForm.slug} onChange={(e) => setPostForm({ ...postForm, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]+/g,'-') })} className="w-full px-3 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-700" />
                      </div>
                      <div className="md:col-span-2">
                        <label className="block text-sm text-gray-700 mb-1">Tóm tắt *</label>
                        <textarea value={postForm.excerpt} onChange={(e) => setPostForm({ ...postForm, excerpt: e.target.value })} rows={2} className="w-full px-3 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-700" />
                      </div>
                      <div className="md:col-span-2">
                        <label className="block text-sm text-gray-700 mb-1">Nội dung *</label>
                        <textarea value={postForm.content} onChange={(e) => setPostForm({ ...postForm, content: e.target.value })} rows={6} className="w-full px-3 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-700" />
                      </div>
                      <div>
                        <label className="block text-sm text-gray-700 mb-1">Tags (phân cách bởi dấu phẩy)</label>
                        <input value={postForm.tags} onChange={(e) => setPostForm({ ...postForm, tags: e.target.value })} className="w-full px-3 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-700" />
                      </div>
                      <div>
                        <label className="block text-sm text-gray-700 mb-1">Trạng thái</label>
                        <select value={postForm.status} onChange={(e) => setPostForm({ ...postForm, status: e.target.value })} className="w-full px-3 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-700">
                      ...
                      <div>
                        <label className="block text-sm text-gray-700 mb-1">Ảnh cover</label>
                        <input type="file" accept="image/*" onChange={(e) => setCoverFile(e.target.files?.[0] || null)} className="w-full px-3 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent cursor-pointer text-gray-700" />
                      </div>
                      <div>
                        <label className="block text-sm text-gray-700 mb-1">Ảnh gallery (nhiều ảnh)</label>
                        <input type="file" accept="image/*" multiple onChange={(e) => setPostFiles(Array.from(e.target.files || []))} className="w-full px-3 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent cursor-pointer text-gray-700" />
                      </div>
                    </div>

                    <div className="mt-6 flex gap-3 items-center">
                      <button disabled={uploading} onClick={createPost} className="px-5 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50">
                        {uploading ? 'Đang tạo...' : 'Tạo bài viết'}
                      </button>
                      <button disabled={uploading} onClick={() => setShowCreatePost(false)} className="px-5 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50">Hủy</button>
                    </div>
                  </div>
                )}
              </div>
